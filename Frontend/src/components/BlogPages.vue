<template>
    <div v-if="blog" class="relative">
      ...
      <div class="text-center mt-20 text-gray-600">
        <p class="text-primary py-4 font-medium">Published On: {{ formatDate(blog.createdAt) }}</p>
        <h1 class="text-2xl sm:text-5xl font-semibold max-w-2xl mx-auto text-gray-800">{{ blog.title }}</h1>
        <h2 class="my-5 max-w-lg truncate mx-auto">{{ blog.content }}</h2>
        <p class="inline-block py-1 px-4 rounded-full mb-6 text-sm border-primary-40 bg-primary-10 font-medium text-primary">
          {{ blog.authorName }}
        </p>
      </div>
      <div class="mx-5 max-w-3xl md:mx-auto my-10 mt-6">
          <img :src="getFullImageUrl(blog.imageUrl)" alt="" class="rounded-3xl mb-5">
...
function getFullImageUrl(url: string | null) {
  return url ? `https://localhost:7137${url}` : ''
}
