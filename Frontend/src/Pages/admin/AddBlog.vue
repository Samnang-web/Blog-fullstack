<template>
  <div class="flex-1 p-4 ml-4 md:p-10 bg-blue-50/50 overflow-scroll items-center	">
    ...
    <form @submit.prevent="submitBlog" class=" bg-blue-50/50 text-gray-600  h-full ">
...
const submitBlog = async () => {
  if (!title.value || !description.value || !content.value || !categoryId.value || !imageFile.value || !authorName.value) {
  ...
  try {
    ...
    authorName.value = ''
    fileInputRef.value.value = null // clear file input
  } catch (error) {
    alert('Error submitting blog: ' + (error.response?.data?.message || error.message))
  }
}
